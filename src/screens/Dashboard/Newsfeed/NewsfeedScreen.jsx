...
const NewsfeedScreen = () => {
	...
	let [posts, setPosts] = useState(newsfeedPosts);
	...
	// Handle select all behaviour in the building filter dropdown
	...
	// Create the array representing the checkbox values
	useEffect(() => {
		setBuildingsCheckedList((buildingsCheckedList = [...buildingsCheckedList]));
	}, [toggleBuildingDropdown]);
	// Set the list of filtered buildings. Any posts with its building not included in the list will NOT be displayed
	useEffect(() => {
	...
	// Get keywords for the flag post feature
	useEffect(() => {
		//get keywords
	...
	return (
		<div className="newsfeed-page">
			<div className="searchBar">
				...
				<Link
					to="/dashboard/approve-newsfeed-posts"
					onClick={() => {}}
					style={{
						display: unapprovedNewsfeedPosts
							? unapprovedNewsfeedPosts.length === 0
								? "none"
								: ""
							: "",
					}}>
					<button className="main approve-new">
						<span className="notification-number">
							{unapprovedNewsfeedPosts ? unapprovedNewsfeedPosts.length : 0}
						</span>
						Approve new
					</button>
				</Link>
